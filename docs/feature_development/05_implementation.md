...
【3 の内訳】
- ReplaceFields を差分更新にする
  今は全削除→再作成のため、ノートが使う項目を消せず失敗する
  id があれば UPDATE、無ければ INSERT、消えたものだけ DELETE
- 使用中の項目の変更・削除は usecase で拒否して 400 を返す
- 画面でも項目を非活性にする（名前とURLは編集できる）
...
```
04_work_breakdown.md の PR6 を実装してください。

【変更するファイル】
- frontend/src/features/note/ 配下

【作るもの】
1. notionPageUrl があれば「Notionで開く」リンクを出す
2. テンプレートに親ページが未設定なら、公開ボタンを非活性にする
   - 理由を画面に出すこと（押せない理由が分からないのが一番困る）
3. 連携に失敗したときのエラー表示
   - もう一度押せば再試行になることを伝える
...
【確認】
□ QA-30 親ページ未設定で公開ボタンが非活性、理由が出る
□ QA-23 公開済み未連携ノートに連携ボタンが出る
□ QA-34 処理中はボタンが押せない
□ notionPageUrl が無いときリンクを出さない
```
