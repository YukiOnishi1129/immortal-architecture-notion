...
型が確定すれば、フロント（PR5-6）も**書き始められます**。
生成された型を使うだけなので、バックエンドの完成を待ちません。

> **ただし、マージは PR3 → PR4 → PR5 → PR6 の順です**
> フロントを先にマージすると、**動かない画面が出てしまいます**。
>
> ```
> PR5  PR3が無い → URLを入力しても保存されない
> PR6  PR4が無い → notionPageUrl が常に null（リンクが出ない）
> ```
>
> 型は揃っているのでビルドは通ります。**動かして初めて気づきます。**
...
note.tsp
  ...
  POST /notes/{id}/notion-sync   ← 手動連携（第2章⑦）
```

**モデルだけでなく、必要な操作も洗い出します。**
第2章⑦で「手動で連携できるようにする」と決めた以上、
そのエンドポイントもここで確定させます。
...
**使用中テンプレートでも保存できるようにします。**

```
既存: ReplaceFields が項目を全削除→再作成する
      → ノートが使う項目を消せずDBエラー
      → 画面が「使用中は編集不可」で回避していた
      → 設定したいテンプレートほど設定できない

対応: 未使用なら従来どおり全削除→再作成
      使用中は SyncFields で id を保って更新
      （どちらを使うかは UseCase が決める）
      使用中の項目の変更・削除は400で拒否
```
...
**手動連携（第2章⑦）もここで実装します。** 同じ usecase の処理で、
PR1-b で契約は決まっています。

```
SyncToNotion   statusは変えず、公開済み & 未連携のときだけ連携する
```
...
□ QA-23（公開済み未連携のノートを notion-sync で連携できる）が通る
```

### PR5 フロント: テンプレート画面

```
変更: frontend/src/features/template/（+150行）
依存: PR3（親ページURLが保存できる）
```
...
□ 未入力でも保存できる（optional）
□ QA-14 使用中テンプレートにも設定できる
□ QA-37 使用中の項目は画面で変更できない
```

> **当初は「入力欄を足すだけ」の想定でした**
> 使用中テンプレートが編集画面に入れないことに、実装中に気づきました。
> 第2章 Step 2 で `ReplaceFields` の中身を読んでいれば拾えた見落としです。

### PR6 フロント: ノート詳細画面

```
変更: frontend/src/features/note/（+180行）
依存: PR4（notionPageUrl と notion-sync が動く）
```
...
> **バックエンドは揃っています**
> 契約は PR1-b、実装は PR4 で済んでいるので、ここは画面だけです。
...
Day 3   PR2-a ／ PR2-b            ├→ PR5 / PR6 を書き始める
Day 4   PR3 →→→→→→→→→→→→→→→→→→→→→→→→ PR5 をマージ
Day 5   PR4 →→→→→→→→→→→→→→→→→→→→→→→→ PR6 をマージ
Day 6   結合して確認
```

**書くのは並行、マージは順番。** フロントは型さえあれば書けますが、
対応するバックエンドが入るまでマージしません。
